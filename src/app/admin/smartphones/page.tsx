...
import { Smartphone, Plus, Trash2, X, Sparkles } from 'lucide-react';
...
export default function SmartphonesAdminPage() {
  ...
  const [newProductWarranty, setNewProductWarranty] = useState('1 Year Warranty');
  const [newProductDescription, setNewProductDescription] = useState('');
  const [isGeneratingAi, setIsGeneratingAi] = useState(false);

  const handleGenerateAiDescription = async () => {
    if (!newProductName.trim()) {
      showToast('Please enter the Smartphone Model Name first to generate an AI description.', 'error');
      return;
    }
    ...
    try {
      const res = await fetch('http://localhost:5000/api/products/generate-description', {
        ...
        body: JSON.stringify({
          ...
          specifications: { warranty: newProductWarranty }
        }),
      });
  ...
  const fetchDbData = async () => {
    try {
      ...
      if (pRes.ok) {
        ...
        const normalized = prods.map((item: any) => ({
          id: item._id || item.id,
          ...
          ramVariants: item.ramVariants || [],
          storageVariants: item.storageVariants || [],
  ...
  const handleAddProductSubmit = async (e: React.FormEvent) => {
    ...
    if (!newProductName || !newProductName.trim()) {
      showToast('Please enter a smartphone model name.', 'error');
      return;
    }

    // 2. Validate Brand
    if (!newProductBrand && brandsList.length === 0) {
      showToast('Please select or add a brand partner first.', 'error');
      return;
    }
    ...
    if (variantImages.length === 0) {
      showToast('Please upload at least one image file for the product variant.', 'error');
      return;
    }
    ...
    const autoRam = Array.from(new Set(variantStockRows.map((r) => r.ram.trim()).filter(Boolean)));
    const autoStorage = Array.from(new Set(variantStockRows.map((r) => r.storage.trim()).filter(Boolean)));

    const parsedColors = autoColors.length > 0 ? autoColors : ['Default'];
    const parsedRam = autoRam.length > 0 ? autoRam : ['8GB'];
    const parsedStorage = autoStorage.length > 0 ? autoStorage : ['128GB'];
    ...
    const payload = {
      ...
      ramVariants: parsedRam,
      storageVariants: parsedStorage,
    ...
    try {
      ...
      if (res.ok) {
        showToast(`Smartphone "${newProductName}" added successfully to MongoDB!`, 'success');
        setNewProductName('');
        ...
        const errData = await res.json().catch(() => ({ message: 'Failed to add product' }));
  ...
  const confirmExecuteRemoveProduct = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showToast(`Smartphone "${prodName}" removed successfully!`, 'info');
        await fetchDbData();
      } else {
        showToast('Failed to remove product from MongoDB', 'error');
      }
    } catch (_) {}
  ...
  const totalPages = Math.ceil(smartphonesList.length / pageSize) || 1;
  const paginatedSmartphones = smartphonesList.slice(
  ...
  return (
    <div className={styles.tableCard} style={{ background: 'var(--card-bg)', borderRadius: 'var(--radius-md)', padding: '24px', border: '1px solid var(--border-color)' }}>
      <div className={styles.tableHeaderBar}>
        <div>
          <h2>Smartphones Inventory ({smartphonesList.length} items)</h2>
          <span style={{ fontSize: '12px', color: 'var(--foreground-secondary)' }}>Manage smartphone models, colors, RAM, storage, prices, and per-variant stock levels.</span>
        </div>
        <button 
          onClick={() => { setVariantStockRows([]); setShowAddProductModal(true); }}
          className="btn btnPrimary"
          style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}
        >
          <Plus size={16} /> Add Smartphone Model
        </button>
      </div>
      ...
      <div className={styles.searchAndFilter} style={{ margin: '16px 0', flexWrap: 'wrap' }}>
        <input 
          type="text" 
          placeholder="Search smartphones by model or brand..." 
      ...
      {paginatedSmartphones.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', background: 'var(--bg-secondary)', borderRadius: '8px', border: '1px dashed var(--border-color)', margin: '16px 0' }}>
          <Smartphone size={36} style={{ color: 'var(--foreground-secondary)', marginBottom: '8px' }} />
          <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--foreground-secondary)', margin: 0 }}>
            No smartphones found. Click "+ Add Smartphone Model" to add your first real device to MongoDB!
          </p>
        </div>
      ) : (
        <div className={styles.tableWrap}>
          <table className={styles.adminTable}>
            <thead>
              <tr>
                <th className={styles.adminTh}>SMARTPHONE MODEL</th>
            ...
            <tbody>
              {paginatedSmartphones.map((prod: any) => (
                <tr key={prod.id || prod._id}>
                  <td className={styles.adminTd}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      {prod.images?.[0] && (
                        <img src={prod.images[0]} alt={prod.name} style={{ width: '36px', height: '36px', objectFit: 'cover', borderRadius: '6px' }} />
                      )}
                      <div>
                        <span style={{ fontWeight: 700, display: 'block' }}>{prod.name}</span>
                        <span style={{ fontSize: '11px', color: 'var(--foreground-secondary)' }}>
                          {prod.variants?.length > 0 ? `${prod.variants.length} Variant Combinations` : 'Base Model'}
                        </span>
                      </div>
                    </div>
                  </td>
      ...
      {smartphonesList.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px', fontSize: '12px', color: 'var(--foreground-secondary)' }}>
          <span>Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, smartphonesList.length)} of {smartphonesList.length} smartphone entries</span>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent} style={{ maxWidth: '750px', background: 'var(--card-bg)' }}>
            <div className={styles.modalHeader}>
              <h2>Add Smartphone Model to Catalog</h2>
              <button onClick={() => setShowAddProductModal(false)} className={styles.modalClose}><X size={20} /></button>
            </div>
            <form onSubmit={handleAddProductSubmit} className={styles.formFields}>
              <div className={styles.formRow}>
                <div className={styles.inputGroup}>
                  <label>Smartphone Model Name</label>
                  <input 
                    type="text" 
                    placeholder="e.g. Vivo V30 Pro or Samsung S24" 
              ...
              <div className={styles.inputGroup}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <label style={{ margin: 0 }}>Smartphone Overview & Description</label>
                  <button
                    type="button"
                    ...
                    style={{
                      ...
                      alignItems: 'center',
                      gap: '6px',
                      background: 'linear-gradient(135deg, #0057FF, #7000FF)',
                      color: '#FFFFFF',
                      border: 'none',
                      padding: '5px 12px',
                      borderRadius: '20px',
                ...
                <textarea 
                  placeholder="Detailed smartphone overview, camera setup, display features, battery capacity..." 
                  ...
                  {variantStockRows.map((row) => (
                    <div key={row.id} style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', padding: '10px', background: 'var(--bg-primary)', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
                      <div style={{ flex: '1 1 120px' }}>
                        <span style={{ fontSize: '10px', fontWeight: 600 }}>Color Variant</span>
                        <input 
                          type="text" 
                      ...
                      <div style={{ flex: '1 1 80px' }}>
                        <span style={{ fontSize: '10px', fontWeight: 600 }}>RAM</span>
                        <select 
                          value={row.ram} 
                          onChange={(e) => handleUpdateVariantRow(row.id, 'ram', e.target.value)}
                          style={{ padding: '6px', fontSize: '12px', width: '100%' }}
                        >
                      ...
                      <div style={{ flex: '1 1 80px' }}>
                        <span style={{ fontSize: '10px', fontWeight: 600 }}>Storage</span>
                        <select 
                          value={row.storage} 
                          onChange={(e) => handleUpdateVariantRow(row.id, 'storage', e.target.value)}
                          style={{ padding: '6px', fontSize: '12px', width: '100%' }}
                        >
                      ...
                      <div style={{ flex: '1 1 150px' }}>
                        <span style={{ fontSize: '10px', fontWeight: 600 }}>Separate Color Image</span>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent} style={{ maxWidth: '420px', textAlign: 'center', background: 'var(--card-bg)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginTop: '16px' }}>
              ...
              <button onClick={confirmExecuteRemoveProduct} className="btn btnPrimary" style={{ background: '#ef4444' }}>Remove</button>
            </div>
          </div>
        </div>
      )}
